...
class Magikarp(object):
	...
	def rand_weights(self, n_in, n_out):
		return tf.random.uniform([n_in, n_out], -1*np.sqrt(6.0/(n_in + n_out)), np.sqrt(6.0/(n_in + n_out)))
	...
	def get_gen_params(self):
		...
		self.g_biases = {
			'b1': tf.Variable(tf.random.normal([self.n_hidden1], stddev=0.01), name='g_b1'),
			'b2': tf.Variable(tf.random.normal([self.n_hidden2], stddev=0.01), name='g_b2'),
			'out': tf.Variable(tf.random.normal([self.n_out], stddev=0.01), name='g_b_out')
		}
	...
	def get_dis_params(self):
		...
		self.d_biases = {
			'b1': tf.Variable(tf.random.normal([self.n_hidden1], stddev=0.01), name='d_b1'),
			'b2': tf.Variable(tf.random.normal([self.n_hidden2], stddev=0.01), name='d_b2'),
			'out': tf.Variable(tf.random.normal([self.n_out], stddev=0.01), name='d_b_out')
		}
	...
	def set_optimization(self):
		...
		self.rand_diff = -1*tf.reduce_mean(tf.math.log(tf.sigmoid(tf.multiply((self.s_pred - self.r_pred), self.playing))))

		# Compute -log(sigmoid(sum of boards)) and -log(sigmoid(- sum of boards))
		self.equal_board1 = -1*tf.reduce_mean(tf.math.log(tf.sigmoid(self.pred_sum)))
		self.equal_board2 = -1*tf.reduce_mean(tf.math.log(tf.sigmoid(-1*self.pred_sum)))
	...
	def create_gen_model(self):
		...
		# Set up graph inputs
		self.f_board_input = tf.compat.v1.placeholder(tf.float32, [None, self.n_input])
		self.s_board_input = tf.compat.v1.placeholder(tf.float32, [None, self.n_input])
		self.r_board_input = tf.compat.v1.placeholder(tf.float32, [None, self.n_input])
		self.results = tf.compat.v1.placeholder(tf.float32, [None, 1])
		self.playing = tf.compat.v1.placeholder(tf.float32, [None, 1])
		self.move_props = tf.compat.v1.placeholder(tf.float32, [None, 1])
		self.p_keep = tf.compat.v1.placeholder(tf.float32)
	...
	def create_dis_model(self):
		...
		self.person_board_1 = tf.compat.v1.placeholder(tf.float32, [None, self.n_input])
		self.person_board_2 = tf.compat.v1.placeholder(tf.float32, [None, self.n_input])
